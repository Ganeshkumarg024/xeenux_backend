...
/**
 * Recalculate user ranks for all users or a specific user
 */
exports.recalculateUserRanks = catchAsync(async (req, res, next) => {
  const { userId } = req.body;
  
  // If userId is provided, only update that specific user
  if (userId) {
    const result = await updateUserRank(userId);
  ...
  for (const user of users) {
    try {
      const result = await updateUserRank(user.userId);
      results.push(result);
    } catch (error) {
      logger.error(`Error updating rank for user ${user.userId}:`, error);
      results.push({
        userId: user.userId,
        status: 'error',
  ...
  res.status(200).json({
    status: 'success',
    data: {
      updatedUsers: results.filter(r => r.updated).length,
      results
    }
  });
});
...
async function updateUserRank(userId) {
  ...
  const teamStructure = await TeamStructure.findOne({ userId: user.userId });
